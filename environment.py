...
class Lift(ManipulationEnv):
    ...
    def reward(self, action=None):
        ...
        # use a shaping reward
            ...
            dist = self._gripper_to_target(
                gripper=self.robots[0].gripper, target=self.sphere.root_body, target_type="body", return_distance=True
            )
            reaching_reward = 1 - np.tanh(10.0 * dist)
            reward += reaching_reward

            # grasping reward
            if self._check_grasp(gripper=self.robots[0].gripper, object_geoms=self.sphere):
                reward += 0.25
    ...
    def _setup_observables(self):
        ...
        if self.use_object_obs:
            ...
            sensors = [sphere_pos, sphere_quat]
            ...
            sensors += [
                self._get_obj_eef_sensor(full_pf, "sphere_pos", f"{arm_pf}gripper_to_sphere_pos", modality)
    ...
    def _check_success(self):
        ...
        """
        sphere_height = self.sim.data.body_xpos[self.sphere_body_id][2]
        table_height = self.model.mujoco_arena.table_offset[2]

        # cube is higher than the table top above a margin
        return sphere_height > table_height + 0.04